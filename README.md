<div align="left">
    <a href="https://www.facebook.com/x3ef8">
        <img src="https://img.shields.io/badge/-x3ef8-blue?style=flat&logo=facebook&logoColor=white&link=https://facebook.com/0x3ef8.dev" alt="Stalk Me, also click FOLLOW button">
    </a>
    <a href="https://user-badge.committers.top/philippines/0x3EF8">
        <img src="https://user-badge.committers.top/philippines/0x3EF8.svg" alt="committers.top badge">
    </a>
    <img align="right" src="https://komarev.com/ghpvc/?username=0x3ef8&label=Profile%20Views&color=565f89&style=flat" alt="Views">
</div>

<div align="center">
    <img width="100%" src="https://github.com/0x3EF8/0x3EF8/blob/main/images/header_.png" />
</div>

```typescript
const developer = {
  name:       "Jay Patrick Cano",
  alias:      "0x3EF8",
  role:       "Full-Stack Developer",
  location:   "Philippines 🇵🇭",
  status:     "Open to opportunities · Always learning",

  stack: {
    languages:  ["JavaScript", "TypeScript", "Python", "PHP", "C#"],
    frontend:   ["React", "Next.js", "Vue.js", "Tailwind CSS", "Sass"],
    backend:    ["Node.js", "Express.js", "Laravel", "REST APIs", "GraphQL"],
    databases:  ["MongoDB", "MySQL", "PostgreSQL", "Firebase"],
    devOps:     ["Git", "Docker", "GitHub Actions", "CI/CD", "AWS · GCP · Azure"],
    automation: ["Python Scripting", "Web Scraping", "Shell & Batch Scripts"],
  },

  links: {
    portfolio:  "https://jaypatrickcano.vercel.app",
    github:     "https://github.com/0x3ef8",
    linktree:   "https://0x3ef8.github.io/linktree",
  },

  quote: "Programming knowledge is the key to unlocking the power of a god.",
} as const;

export default developer;
```

<!-- STATS:START -->
```text
0x3EF8 · Dev Metrics                                                     | Quick Insights
From: 2020 - To: 2026   |   47+ public repos   |   52 stars              | Top Lang : TypeScript (44.99%)
WakaTime (last 7d): 18 hrs 24 mins total · 3 hrs 4 mins daily avg        | Peak Time: Night (52.97%)
                                                                         | Peak Day : Monday (32.91%)
                                                                         | Activity : 44 chunks
├── Stats & Proficiency
│   ├── Languages
│   │   ├── TypeScript        ▰▰▰▰▱▱▱▱▱▱   44.99 %   |  8.32 h   |   /\_/\
│   │   ├── Python            ▰▰▱▱▱▱▱▱▱▱   19.69 %   |  3.64 h   |  ( o.o )
│   │   ├── Markdown          ▰▱▱▱▱▱▱▱▱▱   13.24 %   |  2.45 h   |   > ^ <
│   │   ├── SQL               ▰▱▱▱▱▱▱▱▱▱    7.81 %   |  1.44 h   | Tests turn fear into speed.
│   │   ├── JSON              ▱▱▱▱▱▱▱▱▱▱    4.31 %   |  0.80 h   | Consistency compounds quality.
│   │   ├── Bash              ▱▱▱▱▱▱▱▱▱▱    4.15 %   |  0.77 h   | Simple code survives long-t...
│   │   ├── Text              ▱▱▱▱▱▱▱▱▱▱    2.48 %   |  0.46 h   | Ship small. Learn fast.
│   │   ├── Git Config        ▱▱▱▱▱▱▱▱▱▱    1.48 %   |  0.27 h   | Readable code scales teams.
│   ├── I Code Most During
│   │   ├── Morning    (06-12)   ▱▱▱▱▱▱▱▱▱▱    1.65 %   |  0.29 h   | Plan and warm up.
│   │   ├── Daytime    (12-18)   ▰▰▱▱▱▱▱▱▱▱   18.88 %   |  3.29 h   | Build and iterate.
│   │   ├── Evening    (18-24)   ▰▰▰▱▱▱▱▱▱▱   26.49 %   |  4.62 h   | Feature flow window.
│   │   ├── Night      (00-06)   ▰▰▰▰▰▱▱▱▱▱   52.97 %   |  9.24 h   | Deep focus zone.
│   ├── I Am Most Productive On
│   │   ├── Monday     ▰▰▰▱▱▱▱▱▱▱   32.91 %   |  5.74 h   | Review day
│   │   ├── Tuesday    ▰▰▱▱▱▱▱▱▱▱   24.90 %   |  4.34 h   | Automation day
│   │   ├── Wednesday  ▰▱▱▱▱▱▱▱▱▱    7.25 %   |  1.26 h   | Learning day
│   │   ├── Thursday   ▰▰▰▱▱▱▱▱▱▱   27.18 %   |  4.74 h   | Planning day
│   │   ├── Friday     ▱▱▱▱▱▱▱▱▱▱    3.56 %   |  0.62 h   | Momentum day
│   │   ├── Saturday   ▱▱▱▱▱▱▱▱▱▱    0.00 %   |     n/a   | Shipping day
│   │   ├── Sunday     ▱▱▱▱▱▱▱▱▱▱    4.20 %   |  0.73 h   | Refactor day
│   ├── Editors and Operating Systems
│   │   ├── VS Code           ▰▰▰▰▰▰▰▰▰▰   100.00 %   | 18.49 h   | Keyboard-first workflow.
│   │   ├── Windows           ▰▰▰▰▰▰▰▰▰▰   100.00 %   | 18.49 h   | Stable dev environment.
│   └── Projects (by repo category)
│       ├── AI & Automation   ▰▰▰▰▱▱▱▱▱▱   36.17 %   | 17 repos · Utility and ops...
│       ├── Web Development   ▰▱▱▱▱▱▱▱▱▱   14.89 %   |  7 repos · Community-facing...
│       ├── Tools & Scripts   ▰▱▱▱▱▱▱▱▱▱    8.51 %   |  4 repos · Long-term invest...
│       └── Bots & Messenger  ▰▱▱▱▱▱▱▱▱▱   14.89 %   |  7 repos · Core growth lane.
└── Languages/Time/Day/Editors/OS from WakaTime API · Projects from GitHub API · Updated: 2026-04-03 00:59:55 PHT
```
<!-- STATS:END -->
